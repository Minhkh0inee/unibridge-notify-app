...
export async function verifyMedicationPhoto(
  ...
  try {
    ...
    const containsMedication = data.contains_medication ?? data.containsMedication;
    const confidence = data.confidence;

    console.log('[MedicationVerification] result:', { containsMedication, confidence });

    if (
      typeof containsMedication !== 'boolean' ||
      (confidence !== 'high' && confidence !== 'medium' && confidence !== 'low')
    ) {
      throw new MedicationVerificationError('Verification API returned an invalid response.');
    }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new MedicationVerificationError('Medication verification timed out.');
    }
    throw new MedicationVerificationError('Unable to reach the medication verification API.');
  } finally {
